...
const MedecinsModule = (() => {
  ...
  Modal.loading('Nouveau medecin');
  const hRes=await Api.Hopitaux.getAll();
  Modal.open('Nouveau medecin',form(null,hRes.data),'modal-lg');
}
      ...
      <div class="form-row">
        ...
        <div class="form-group"><label class="form-label">Hopital *</label>
          <select class="form-control" id="mf-hop">
            ...
            ${hops.map(h=>`<option value="${h.id}" ${Number(v.hopital_id)==h.id?'selected':''}>${h.nom}</option>`).join('')}
          </select></div>
      </div>
      <div class="form-row">
  ...
  async function save(id){
    ...
    const data={prenom,nom,specialite:spec,hopital_id:Number(hopId),telephone:document.getElementById('mf-tel').value.trim(),email:document.getElementById('mf-email').value.trim(),horaires};
