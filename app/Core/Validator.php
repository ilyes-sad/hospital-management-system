...
class Validator
{
    ...
    private function applyRule(string $field, string $rule): void
    {
        ...
        match ($ruleName) {
            ...
            'cin_format' => $this->validateCinFormat($field, $value),
            default    => null,
        };
    }
    ...
    private function validateCinFormat(string $field, mixed $value): void
{
    if ($value !== null && trim((string) $value) !== '') {
        if (!preg_match('/^\d{8}$/', trim((string) $value))) {
            $this->addError($field, 'Le CIN doit contenir exactement 8 chiffres.');
        }
    }
}
