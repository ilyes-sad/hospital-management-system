...
// SIMPLE TEST - if no routing, just show this
echo "<h1>MediCare Works!</h1>";
...
echo "<hr><h2>Now loading router...</h2>";
...
$strippedUri = preg_replace('#^' . preg_quote($scriptDir, '#') . '#', '', $uri);
if ($scriptName && strpos($strippedUri, $scriptName) === 0) {
    $strippedUri = substr($strippedUri, strlen($scriptName));
}
$strippedUri = rtrim($strippedUri, '/');
if ($strippedUri === '') $strippedUri = '/';

echo "<!-- DEBUG: original_uri=$uri, scriptDir=$scriptDir, scriptName=$scriptName, final_uri=$strippedUri -->";
